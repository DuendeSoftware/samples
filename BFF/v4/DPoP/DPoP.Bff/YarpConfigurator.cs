...
public static class YarpConfigurator
{
    ...
    public static void MapRemoteUrls(this WebApplication app)
    {
        ...
        app.MapRemoteBffApiEndpoint("/api/client-token", new Uri("https://localhost:6001"))
            .WithAccessToken(RequiredTokenType.Client);

        // On this path, we use a user token if logged in, and fall back to a client credentials token if not
        app.MapRemoteBffApiEndpoint("/api/user-or-client-token", new Uri("https://localhost:6001"))
            .WithAccessToken(RequiredTokenType.UserOrClient);

        // On this path, we make anonymous requests
        app.MapRemoteBffApiEndpoint("/api/anonymous", new Uri("https://localhost:6001"));

        // On this path, we use the client token only if the user is logged in
        app.MapRemoteBffApiEndpoint("/api/optional-user-token", new Uri("https://localhost:6001"))
            .WithAccessToken(RequiredTokenType.UserOrNone);

        // On this path, we require the user token
        app.MapRemoteBffApiEndpoint("/api/user-token", new Uri("https://localhost:6001"))
