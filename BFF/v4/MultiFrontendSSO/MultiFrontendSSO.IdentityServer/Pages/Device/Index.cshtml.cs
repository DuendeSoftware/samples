...
public class Index : PageModel
{
    ...
    public async Task<IActionResult> OnPost()
    {
        ...
        else if (Input.Button == "yes")
        {
            ...
            if (Input.ScopesConsented.Any())
            {
                ...
                {
                    scopes = scopes.Where(
                        x => x != Duende.IdentityServer.IdentityServerConstants.StandardScopes.OfflineAccess);
                }
    ...
    private ViewModel CreateConsentViewModel(DeviceFlowAuthorizationRequest request)
    {
        ...
        vm.IdentityScopes = request.ValidatedResources.Resources.IdentityResources.Select(
                x => CreateScopeViewModel(x, Input == null || Input.ScopesConsented.Contains(x.Name)))
            .ToArray();
