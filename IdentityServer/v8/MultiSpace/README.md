...
This sample demonstrates [Duende MultiSpace](Documentation.md) — running multiple isolated IdentityServer spaces within a single ASP.NET Core host. Each space has its own users, clients, identity providers, and configuration data, completely isolated from other spaces.
...
```
MultiSpace/
├── src/MultiSpace/              # IdentityServer host + Razor Pages UI
...
├── tests/MultiSpace.PlaywrightTests/  # E2E browser tests
├── aspire/AppHost/              # Aspire orchestration (used by tests)
├── Documentation.md             # MultiSpace feature documentation
...
MultiSpace resolves the current space early in the middleware pipeline via `app.UseMultiSpaceResolution()`. This must run before `app.UseIdentityServer()` so that all IdentityServer operations use the correct space context.
...
pwsh tests/MultiSpace.PlaywrightTests/bin/Debug/net10.0/.playwright/package/bin/playwright.ps1 install

# Run tests
dotnet test tests/MultiSpace.PlaywrightTests
```
## Further Reading

- [MultiSpace Documentation](Documentation.md) — Full feature documentation including configuration options, space resolution, and management APIs
