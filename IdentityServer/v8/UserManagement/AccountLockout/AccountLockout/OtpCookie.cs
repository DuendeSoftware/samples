...
{
    private const string CookieName = "__Host-AccountLockoutOtpAuthentication";

    private readonly IDataProtector _protector = dataProtectionProvider.CreateProtector("AccountLockout.OtpAuthentication.v1");
    ...
    public bool TryRead([NotNullWhen(true)] out OtpToken? token, [NotNullWhen(true)] out EmailAddress? emailAddress)
    {
        token = null;
        emailAddress = null;

        if (!HttpContext.Request.Cookies.TryGetValue(CookieName, out var protectedJson) || string.IsNullOrWhiteSpace(protectedJson))
        {
            return false;
        }
