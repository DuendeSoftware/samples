...
public class AcrDiscoveryDocumentGenerator : DiscoveryResponseGenerator
{
    ...
    public override async Task<Dictionary<string, object>> CreateDiscoveryDocumentAsync(string baseUrl, string issuerUri, CancellationToken cancellationToken)
    {
        var result = await base.CreateDiscoveryDocumentAsync(baseUrl, issuerUri, cancellationToken);
