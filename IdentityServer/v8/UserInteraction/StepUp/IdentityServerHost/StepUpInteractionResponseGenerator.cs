...
public class StepUpInteractionResponseGenerator : AuthorizeInteractionResponseGenerator
{
    ...
    protected override async Task<InteractionResponse> ProcessLoginAsync(ValidatedAuthorizeRequest request, CancellationToken cancellationToken)
    {
        var result = await base.ProcessLoginAsync(request, cancellationToken);
