...
public class SpaEndpoints : ControllerBase
{
    ...
    {
        var authzContext = await _interaction.GetAuthorizationContextAsync(returnUrl, HttpContext.RequestAborted);
    ...
    public async Task<IActionResult> Login([FromBody] LoginRequest model)
    {
        ...
        if (ModelState.IsValid && _users.ValidateCredentials(model.Username, model.Password))
        {
            ...
            var authzContext = await _interaction.GetAuthorizationContextAsync(url, HttpContext.RequestAborted);
            if (authzContext != null)
            {
    ...
    {
        var errorInfo = await _interaction.GetErrorContextAsync(errorId, HttpContext.RequestAborted);
        return Ok(new
        {
            errorInfo.Error,
    ...
    {
        var logoutInfo = await _interaction.GetLogoutContextAsync(logoutId, HttpContext.RequestAborted);
